...
const CombinedBidForm = () => {
  ...
  const handleBidSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(true); // Show OTP verification modal
      setError('');
    } catch (err) {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      setOtpMessage('Bid verified and submitted successfully!');
      setTimeout(() => {
        setShowModal(false);
        navigate('/user');
      }, 2000);
  ...
  return (
    <div className=" bg-white p-4 sm:p-6 lg:p-8 flex flex-col">
